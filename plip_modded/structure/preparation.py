...
from plip_modded.basic import config, logger
from plip_modded.basic.supplemental import centroid, tilde_expansion, tmpfile, classify_by_name
from plip_modded.basic.supplemental import cluster_doubles, is_lig, normalize_vector, vector, ring_is_planar
from plip_modded.basic.supplemental import extract_pdbid, read_pdb, create_folder_if_not_exists, canonicalize
from plip_modded.basic.supplemental import read, nucleotide_linkage, sort_members_by_importance
from plip_modded.basic.supplemental import whichchain, whichrestype, whichresnumber, euclidean3d, int32_to_negative
from plip_modded.basic.supplemental import residue_belongs_to_receptor
from plip_modded.structure.detection import halogen, pication, water_bridges, metal_complexation
from plip_modded.structure.detection import hydrophobic_interactions, pistacking, hbonds, saltbridge
...
class PLInteraction:
    ...
    def refine_water_bridges(wbridges, hbonds_ldon, hbonds_pdon):
        ...
        # (TODO: looks wrong, what about donor atoms with multiple H? Does plip_modded cover that somehow?
...
class PDBComplex:
    ...
    sites as well as information about the plip_moddedrofiler between them. Provides functions to load and prepare input files
    such as PDB files.
    """
        ...
        if not config.PLUGIN_MODE:
            if pdbparser.num_fixed_lines > 0:
                ...
                if not as_string:
                    ...
                    basename = "from_stdin"
                pdbpath_fixed = tmpfile(prefix='plip_moddedfixed.' + basename + '_', direc=self.output_path)
