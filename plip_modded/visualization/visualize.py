...
File edited by mattialpg in order to save a pse of the binding site only
...
from plip_modded.basic.supplemental import start_pymol
from plip_modded.visualization.pymol import PyMOLVisualizer

logger = logger.get_logger()


def visualize_in_pymol(plcomplex, residues_to_keep=None):
    """
    Visualizes the given Protein-Ligand complex at one site in PyMOL.
    If residues_to_keep is provided, only ligand + those residues are kept.
    """
    ...
    ###! EDITED: restrict to ligand + interacting residues if requested
    if residues_to_keep:
        lig_sel = f"resn {hetid} and chain {chain} and resi {plcomplex.position}"
        res_sel = " or ".join([f"(resi {r[:-1]} and chain {r[-1]})" for r in residues_to_keep])
        cmd.remove(f"not ({lig_sel} or {res_sel})")
        cmd.show('sticks', 'all')

        # Translate system COM to the origin
        com = cmd.centerofmass("all")
        x, y, z = com
        cmd.translate([-x, -y, -z], "all")
    # ###! END EDITED
