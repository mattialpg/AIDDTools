...
Edited by mattialpg in order to save an eps of the binding site only
...
class PyMOLVisualizer:
    ...
    def selections_cleanup(self):
        ...
        cmd.delete('tmp*')
        cmd.delete('ps-*')
        cmd.delete('Hydrophobic-P HBondAccept-P HBondDonor-P HalogenAccept '
                           'StackRings-P PosCharge-P NegCharge-P AllBSRes '
                           'Hydrophobic-L HBondAccept-L HBondDonor-L HalogenDonor NegCharge-L '
                           'PosCharge-L NegCharge-L StackRings-L Metal-L Metal-M '
                           'Unpaired-HBA Unpaired-HBD Unpaired-HAL Unpaired-RINGS')

    def selections_group(self):
        """Group all selections"""
        cmd.group('Complex', '%s' % self.protname)
        cmd.group('Complex', 'Hydrophobic HBonds HalogenBonds WaterBridges PiCation PiStackingP PiStackingT '
                                  'Saltbridges MetalComplexes')
        cmd.group('Complex', 'Hydrophobic-P HBondAccept-P HBondDonor-P HalogenAccept Centroids-P PiCatRing-P '
                           ...
                           'Unpaired-HBA Unpaired-HBD Unpaired-HAL Unpaired-RINGS Water Metal-W')
        cmd.order('*', 'y')

    def additional_cleanup(self):
        """Cleanup of various representations"""
        cmd.remove('not alt ""+A')  # Remove alternate conformations
        # cmd.hide('labels', 'Complex.Interactions')  # Hide labels of lines
    ...
    def save_session(self, outfolder, override=None):
        ...
        filename = '%s' % (self.protname.replace('Protein','bindingsite'))
